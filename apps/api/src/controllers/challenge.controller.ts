...
export const getGroupChallenges = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.userId;
...
export const joinChallenge = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const challengeActivities = await prisma.activity.findMany({
            where: {
                userId,
                startTime: {
...
export const leaveChallenge = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const deleted = await prisma.challengeParticipant.deleteMany({
            where: {
                userId,
                challengeId
            }
...
export const getChallengesForUserGroups = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.userId;
        if (!userId) return res.status(401).json({ message: 'Unauthorized' });
...
export const deleteChallenge = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const challengeToCheck = await prisma.challenge.findUnique({
            where: { id },
            include: { group: { include: { members: { where: { userId } } } } }
        });

        if (!challengeToCheck) return res.status(404).json({ message: 'Challenge not found' });
...
export const uploadChallengePhoto = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.userId;
        const id = req.params.id as string;
        const file = req.file;
