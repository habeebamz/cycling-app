...
export const createReport = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (postId) {
            ...
            const activity = await prisma.activity.findUnique({ where: { id: activityId }, include: { user: true } });
            if (!activity) return res.status(404).json({ message: 'Activity not found' });
            targetType = 'activity';
            targetName = activity.user.username;
        } else if (groupId) {
            const group = await prisma.group.findUnique({ where: { id: groupId } });
            if (!group) return res.status(404).json({ message: 'Group/Club not found' });
            targetType = 'group';
...
export const getReports = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const pendingAppeals = await (prisma as any).groupMember.findMany({
        ...
        const mappedAppeals = pendingAppeals.map((appeal: any) => ({
            id: `appeal-${appeal.id}`,
        ...
        // Combine and sort
        const combined = [...reports, ...mappedAppeals].sort((a, b) =>
...
export const deletePostByReport = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.userId;
        const id = req.params.id as string; // report id
...
export const suspendEntityByReport = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.userId;
        const id = req.params.id as string; // report id
...
export const deleteActivityByReport = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.userId;
        const id = req.params.id as string; // report id
...
export const suspendAuthorByReport = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.userId;
        const id = req.params.id as string; // report id
        ...
        let authorUsername = '';
        let groupId: string | null = report.groupId;
        ...
        if (groupId) {
            ...
            if (membership) {
                ...
                let days = 7;
                if (newCount === 2) days = 30;
                else if (newCount >= 3) days = 365;
